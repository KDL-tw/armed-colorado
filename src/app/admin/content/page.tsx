...
export default async function AdminContentPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-4xl space-y-16 px-4 pb-20 pt-28">
      <div>
        <Link href="/admin" className="text-sm text-amber">
          ← Admin
        </Link>
        <h1 className="mt-3 font-display text-4xl text-cream">Content</h1>
      </div>

      <section>
        <h2 className="font-display text-2xl text-cream">FAQs</h2>
        <form action={upsertFaq} className="mt-4 grid gap-3 border border-silver/20 p-4">
          <p className="text-sm text-silver">Add FAQ</p>
        ...
        <ul className="mt-6 space-y-4">
          {faqs.map((faq) => (
            <li key={faq.id ?? faq.question} className="border-t border-silver/15 pt-4">
              <p className="text-cream">{faq.question}</p>
              <p className="mt-1 text-sm text-silver">{faq.answer}</p>
              {faq.id ? (
      ...
      <section>
        <h2 className="font-display text-2xl text-cream">Resources</h2>
        <form action={upsertResource} className="mt-4 grid gap-3 border border-silver/20 p-4">
      ...
      <section>
        <h2 className="font-display text-2xl text-cream">SB25-003 counties</h2>
        <form action={upsertCounty} className="mt-4 grid gap-3 border border-silver/20 p-4">
      ...
      <section>
        <h2 className="font-display text-2xl text-cream">CCW orgs</h2>
        <form action={upsertCcwOrg} className="mt-4 grid gap-3 border border-silver/20 p-4">
          <input name="name" placeholder="Name" required className="admin-input" />
          <input name="url" placeholder="URL" className="admin-input" />
